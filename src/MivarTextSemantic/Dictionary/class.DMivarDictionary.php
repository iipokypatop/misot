<?php
require_once(__DIR__.'/php_morph.helper.php');
require_once(__DIR__.'/dictonary.helper.php');
require_once(__DIR__.'/../classes_wdw/class.MorphAttribute.php');
require_once(__DIR__.'/../classes_wdw/class.Dw.php');

//Класс для работы со словарем по текущему тексту.
...
class DMivarDictionary
{
	...
	function __construct($words_array, $need_forms = false, $connection_string = DB_CONNECTION)
	{
	...
	public function get_words($words_array, $need_forms = false, $use_predict = true, $use_object = true) {
		...
		if (is_array($words_array) && $words_array) {
			...
			if ($use_predict && $this->array_missing_words) {
				$miss_words_predict = getWordFromAllDict($this->array_missing_words);
