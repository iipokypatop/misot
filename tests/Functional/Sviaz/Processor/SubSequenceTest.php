...
class SubSequenceTest extends \AotTest\AotDataStorage
{
     ...
     * @param int[] $sequence_config
     * @param int[][] $sviazi_config
     * @param int[][] $expected_subsequences_config
     */
    public function testDetectSubSequencesInSequenceWithMultiNumberOfGrammaticalFoundations(
        $sequence_config,
        $sviazi_config,
        ...
        $members = [];
        foreach ($sequence_config as $i => $sequence_element_type) {
        ...
        $sviazi = [];
        foreach ($sviazi_config as $i => $sviaz_config) {
            ...
            //Реализуем первый метод
            $main_position = $sviaz_config[0];
            $sviazi[$i]
                ->expects($this->at(0))
                ->method('getMainSequenceMember')
                ->with()
                ->will($this->returnValue($members[$main_position]));

            //Реализуем второй метод
            $depended_position = $sviaz_config[1];
            $sviazi[$i]
                ->expects($this->at(1))
                ->method('getDependedSequenceMember')
                ->with()
                ->will($this->returnValue($members[$depended_position]));
        }
        ...
        foreach ($sub_sequences as $sub_sequence) {
            $res[] = [$sub_sequence->getIndexStart(), $sub_sequence->getIndexEnd()];
        }

        $this->assertEquals($expected_subsequences_config, $res);
    }
    ...
    public function testIsMemberInSequencesTrue()
    {
        ...
        $sub_sequence = \Aot\Sviaz\SubSequence::create($sequence, $length_sequence_part_1 - 2,
